...
export class REST {
	...
	public readonly setupKairos = async (): Promise<void> => {
		...
		try {
			...
			console.log(this.instance.KairosObj.AUX)
		} catch (error) {
		...
		this.instance.KairosObj.SCENES.forEach((scene: any) => {
			scene.layers.forEach((layer: any) => {
				this.instance.combinedLayerArray.push({name:`/${scene.name}/${layer.name}`, sourceA: layer.sourceA, sourceB: layer.sourceB, uuid: layer.uuid})
			})
		})
		console.log(this.instance.combinedLayerArray)
